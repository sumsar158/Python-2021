...
class Student:
    ...
    def __init__(self, name: str, courses: list, avarage_grade: float):
        """Student constructor.

        :param name: Name of the student
        :param courses: Courses student attends
        :param avarage_grade: Students avarage grade
        """
        self.name = name
...
def is_failing(student: Student) -> bool:
    ...
    """
    return student.avarage_grade < 1.0
...
def succeeding_students(student_list: list) -> list:
    ...
    """
    not_failing_students = []

    for student in student_list:
        if not is_failing(student):
            not_failing_students.append(student)

    return not_failing_students
...
def sort_by_best_grade(student_list: list) -> list:
    ...
    """
    return sorted(succeeding_students(student_list), key=lambda student: (student.avarage_grade, student.name),
                  reverse=True)
