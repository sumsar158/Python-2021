...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    for tweet in tweets:
        dict_tweet[tweet] = tweet.ratio

    return max(dict_tweet, key=dict_tweet.get)
...
def sort_by_popularity(tweets: list) -> list:
    ...
    tweets_popularity = []
    for tweet in tweets:
        tweets_popularity.append(tweet)
    tweets_popularity = sorted(tweets_popularity, key=lambda x: (x.retweets, x.time), reverse=True)
